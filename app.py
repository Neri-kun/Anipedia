...
def home():
    ...
    urls = []
    results = generate_query_results()["results"]["bindings"]
    ...
    results = [{**result, "episodes": {"value": new_results[i]}} for i, result in enumerate(results)]

    genres = generate_query_results("","All genres")["results"]["bindings"]
    filtered_genres = list(filter(lambda genre: genre["genre"]["value"].strip(), genres))


    return render_template("anime_website.html",results=results, urls = urls, genres = filtered_genres)
...
def searchPaginated(page, query, genre="", date="", endDate=""):
    ...
    all_results =  generate_query_results(query, genre, date, endDate)['results']['bindings']
    ...
    return render_template('search_results.html', results=results, current_page=page, num_pages=num_pages, has_prev=has_prev, has_next=has_next,
                           query=query, page=page, date=date, endDate=endDate, genre=genre, total_results = total_results)
...
def generate_query_results(query_keyword="", genre="", startDate="", endDate=""):
    ...
    #Set query according to use cases
